(function() {
    ...
    function setQuiz() {
      ...
      for (i = 0; i < answers.length; i++) {
        answers[i].classList.remove('いいいね！');
        answers[i].classList.remove('どうですかね！');
    ...
    function checkAnswer(node) {
      ...
      if (node.textContent === quizSet[currentNum].a[0]) {
        node.textContent += ' ... いいいね!';
        node.classList.add('いいいね!');
        score++;
      } else {
        node.textContent += ' ... どうですかね!';
        node.classList.add('どうですかね');
      }
